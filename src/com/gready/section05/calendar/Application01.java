...
public class Application01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * Deprecated란?
		 * 향후 버전이 업데이트 되며서 사라지겓 될 기능이니 가급적이면 사용을 권장하지 않는다는 의미이다,
		 * 하지만 하위 버전의 호환성 떄문에 한 번에 없애는 것이 아니고 남겨둔 것이니 사용하는것은 가능하긴하다.
		 * 
		 * Calendar 클래스
		 * JDK 1.1부터 새롭게 재공되는 Calendar 클래스를 이용하여 날짜와 시간에 관련된 처리를 하는걸 권장한다.
		 * Calendar 클래스는 몇가지 문제를 가지고 있다.
		 * 
		 * 1. Calendar 인스턴스는 불변객체가 아니기 떄문에 값을 수정할 수 있다.
		 * 2. 윤초(leaf second)를 고려하지 않는다.
		 * 3. Calendar 클래스는 월을 표현할 떄 0~11까지로 표현하는 불편함이 있다.
		 * 
		...
		java.util.Date time = new java.util.Date(1674097405666L);
